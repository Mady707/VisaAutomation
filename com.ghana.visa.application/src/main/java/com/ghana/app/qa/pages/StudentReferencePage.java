...
public class StudentReferencePage extends TestBase{

	public StudentReferencePage() throws IOException, InterruptedException {
	...
	public WebElement StudentSaveAndContinuebtn;
	
	
	
	public void passSchoolName(String name) throws InterruptedException {
		SchoolName.sendKeys(name);
	}
	...
	public void clickStudentContinueBtn() {
		StudentSaveAndContinuebtn.click();
	}
	...
	for (WebElement CountryType : totalCountryType){
		if(CountryType.getText().equalsIgnoreCase(TestUtil.readDataFromExcellString(7, row, 6))){
			CountryType.click();
			}
		}
	}
	...
	for (WebElement StateType : totalStateType){
		Thread.sleep(1000);
		if(StateType.getText().equalsIgnoreCase(TestUtil.readDataFromExcellString(7, row, 7))){
			Thread.sleep(1000);
			StateType.click();
			}
		}
